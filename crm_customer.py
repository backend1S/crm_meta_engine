...
def load_last_cre_map():
    ...
    sql = """
    SELECT BRANCH_ID, MAX(CUSTOMER_ID)
    FROM CRM_CUSTOMERS
    GROUP BY BRANCH_ID
    """
...
def load_existing_customers():
    ...
    sql = "SELECT PHONE_NUMBER, EMAIL FROM CRM_CUSTOMERS"
    status, rows = db.db_query(sql)
    ...
    if status and rows:
        for row in rows:
            if row[0]:
                phones.add(clean_text(row[0], 30))
            if row[1]:
                emails.add(clean_text(row[1], 150).lower())
...
# =========================================================
# BULK INSERT (🔥 FIXED CREATED_DATE)
# =========================================================
def bulk_insert_customers(rows_to_insert):
    if not rows_to_insert:
        return 0
    ...
    log_error(f"❌ CRM insert failed → {result}")
    return 0


# =========================================================
# PROCESS LEADS (🔥 FINAL FIX)
# =========================================================
def process_leads():
    ...
    for row in rows:
        try:
            lead_id = clean_text(row[1])
            name = clean_text(row[13])
            phone = clean_text(row[14])
            email = clean_text(row[15]).lower()

            city_from_lead = clean_text(row[16])
            campaign_name = clean_text(row[8])
            ...
            month = clean_text(row[17])
            contact_mode = get_exact_contact_mode(row[18])
            pax = parse_pax(row[19])
            platform = clean_text(row[12])
    ...
    if processed_ids:
        ...
        sql = f"UPDATE LEADS SET IS_PROCESSED = 1 WHERE LEAD_ID IN ({placeholders})"
        db.db_update(sql, processed_ids)

    log(f"✅ DONE → processed={len(rows)} | inserted={inserted}")
